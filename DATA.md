...
- `rest_oil`에도 동일한 이유로 `admin_overridden`(boolean, `@Column(nullable = false, columnDefinition = "boolean default false")`) 컬럼을 둔다. `rest_stop`↔`rest_oil` 연결은 `route_no = route_code` + 정규화 이름 일치라는 fuzzy 매칭이라 이름이 조금만 달라도 놓치거나 잘못 연결될 수 있는데, 이 매칭은 `RestStopServiceAreaCodeBackfillService.backfillRestOils()`가 매일 자정·3시간마다·서버 시작 시 다시 계산해 `rest_stop_service_area_code`를 덮어쓴다. 관리자가 `PUT /api/admin/oil-stations/{oilId}/link`로 연결을 수정하거나 `DELETE .../link`로 연결을 해제하면(둘 다 `adminOverridden=true`) 이 배치가 해당 행을 건너뛴다. `DELETE /api/admin/oil-stations/{oilId}/override`로 다시 false가 되면 다음 배치부터 정상 매칭된다.
