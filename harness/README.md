...
- `harness/runs/current/state.env`는 경로가 고정돼 있어 다음 작업을 시작하면 덮어써진다(과거 이 때문에 리뷰가 스킵된 사고가 있었다 — 아래 "알려진 리스크" 참고). 커밋이 실제로 완료된 시점에 `04-archive-state.sh`가 그 시점의 `state.env`를 `harness/runs/history/<브랜치명>.env`로 복사해 둔다. PR 번호는 커밋 시점에 아직 모르므로(브랜치와 PR은 보통 1:1) 브랜치명을 키로 쓰며, git으로 추적하지 않는 로컬 전용 기록이다.
