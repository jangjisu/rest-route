...
- 휴게소 비교(`GET /api/rest-stops/compare?serviceAreaCodeA=&serviceAreaCodeB=`)는 이름 검색으로 고른 두 휴게소의 유가 3종·주차 대수·부대시설을 비교한다. `RestStopCompareService`(`service.compare` 패키지)는 기존 `RestStopRelatedInfoQueryService.findByRestStop()`을 양쪽에 한 번씩 호출해 데이터를 모으고, 항목별 승자와 종합 추천 side를 함께 계산해 반환한다 — 프론트엔드는 표시만 하고 승패 판정 로직을 갖지 않는다. 부대시설은 `RestStopDetailEntity.convenience`(`|` 구분)만 사용하고 `RestOilEntity.convenienceName`(주유소 자체 부대시설)은 포함하지 않는다 — 이미 출고된 `RouteRestStopComparisonSummaryService.facilityCount()`가 두 데이터를 섞어 세는 것과 달리, 새 기능은 "그 휴게소" 시설만 좁혀서 비교한다(기존 로직은 이번 스코프에서 고치지 않음). 승자는 가격·주차는 낮거나/많은 쪽, 부대시설은 개수가 많은 쪽이며 동률·데이터 없음은 승자 없음(`null`)으로 취급한다. 이 기능은 이미 저장된 데이터만 사용하며 새로운 외부 API 호출은 없다.
