...
- **목적지로 추천받기의 배지는 이름·거리로 찾기와 같은 4개(규모/이용량/볼거리/이벤트)가 항상 계산되고,
  마지막 한 자리만 관심 항목에 따라 켜진다**: `interest === 'EV'`면 EV 충전(대수), 유종이면 유가 등급
  하나만 붙는다. 조건 필터 칩 구성(`destinationConditionFilters`)은 배지와 다르게 규모·이용량만 항상
  켜고 볼거리·이벤트는 필터 칩으로 노출하지 않는다 — 나머지 한 자리만 관심 항목에 맞춰 보인다(건너뛰었으면
  규모·이용량 칩 2개뿐).
