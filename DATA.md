...
- 주유 가격(3시간 주기)만 모든 페이지 조회가 끝난 뒤 트랜잭션 안에서 기존 데이터를 전체 교체하고, 저장 직후 `rest_oil`의 연결 값을 기준으로 `rest_stop_service_area_code`를 다시 채운다. 일부 페이지만 성공하면 기존 행을 보존한 채 성공 응답만 upsert한 뒤 같은 backfill을 수행한다.
...
- `rest_stop`/`rest_stop_detail`에 `admin_overridden`(boolean, 기본값 false) 컬럼을 둔다. `@Column(nullable = false, columnDefinition = "boolean default false")`로 선언해, 기존 행이 있는 테이블에 `ddl-auto=update`로 컬럼을 추가할 때도(`ALTER TABLE ... ADD COLUMN`) 기존 행이 `false`로 채워져 `NOT NULL` 제약 위반 없이 반영된다. 관리자가 `PUT /api/admin/rest-stops/{serviceAreaCode}/editable`로 편집하면 두 테이블 모두 이 값이 true가 되고, 자연키 upsert 동기화는 이 값이 true인 행을 건너뛴다(자동 갱신에서 제외). `DELETE .../editable/override`로 다시 false가 되면 다음 동기화부터 정상 갱신된다. `data.ex.co.kr` API 자체에 없는 휴게소는 `POST /api/admin/rest-stops`로 관리자가 새로 등록하며, `rest_food`의 `"ADMIN-" + UUID` 발급 방식과 동일하게 `rest_stop.service_area_code`/`unit_code`/`std_rest_cd`를 모두 `"ADMIN-" + UUID`로 채워 실제 API 자연키와 겹치지 않게 하고 생성 시점부터 `admin_overridden=true`로 시작한다.
...
- `rest_stop`↔`rest_oil` 연결은 `route_no = route_code` + 정규화 이름 일치라는 fuzzy 매칭이며, 관리자가 수정한 연결을 보존하기 위한 `admin_overridden`은 기준 데이터인 `rest_oil`에만 둔다. `rest_oil_price.rest_stop_service_area_code`는 `rest_oil.standard_rest_code = rest_oil_price.service_area_code2` 관계로 파생되는 값이라 별도 잠금 상태를 소유하지 않는다. 가격 전체 교체·부분 upsert와 정기 backfill은 모두 `rest_oil` 연결을 기준으로 가격 행을 다시 채우므로 관리자 연결·연결 해제 상태가 가격 동기화와 충돌하지 않는다. `rest_oil`은 물리적 주유소 하나당 부대 편의시설 개수만큼 여러 행이 생기고 `rest_oil_price`는 사용자에게 보이는 업체·가격 정보가 담긴 휴게소당 0~1건이므로, 관리자 화면은 가격 행을 목록·검색 단위로 사용하되 연결·해제·잠금 해제 상태는 같은 `standard_rest_code`의 `rest_oil` 행 전체에 기록한다.
- 기존 운영 DB에 남은 `rest_oil_price.admin_overridden`은 더 이상 애플리케이션에서 읽거나 쓰지 않는다. Hibernate `ddl-auto=update`는 삭제된 엔티티 필드의 물리 컬럼을 제거하지 않으므로 배포 시 `ALTER TABLE rest_oil_price DROP COLUMN admin_overridden;`를 일회성으로 실행한다.
