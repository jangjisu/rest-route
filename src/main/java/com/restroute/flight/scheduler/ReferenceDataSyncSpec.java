...
/**
 * 국가/도시/공항/항공사 4종 참조 데이터 재시딩이 SQL 경로·repository 콜백·로그 라벨만 다르고
 * 나머지 흐름(재시딩 → 성공/실패 로그)은 동일해서, 그 흐름을
...
record ReferenceDataSyncSpec(
    ...
    static ReferenceDataSyncSpec of(
            ...
            Supplier<Long> currentCount,
            String enabledPropertyKey) {
        return new ReferenceDataSyncSpec(label, seedSqlPath, clearExisting, currentCount, enabledPropertyKey);
    }
}
