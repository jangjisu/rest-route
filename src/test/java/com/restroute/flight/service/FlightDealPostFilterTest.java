...
class FlightDealPostFilterTest {
    ...
    private static FlightDealResponse dealAt(
            String id, String departDate, int departureTransfers, int arrivalTransfers) {
    ...
    void apply_removesTransfersWhenIncludeTransferFalse() {
        // 2026-09-14는 월요일(평일)이라 다른 필터에 안 걸린다.
        List<FlightDealResponse> items = List.of(
                dealAt("A", "2026-09-14", 0, 0), dealAt("B", "2026-09-14", 1, 0), dealAt("C", "2026-09-14", 0, 1));
    ...
    void apply_keepsTransfersByDefault() {
        List<FlightDealResponse> items = List.of(dealAt("A", "2026-09-14", 1, 0));
    ...
    void apply_removesWeekendDeparturesByDefault() {
        // 2026-09-12는 토요일, 2026-09-14는 월요일.
        List<FlightDealResponse> items = List.of(dealAt("SAT", "2026-09-12", 0, 0), dealAt("MON", "2026-09-14", 0, 0));
    ...
    void apply_keepsWeekendDepartures_whenExplicitlyIncluded() {
        List<FlightDealResponse> items = List.of(dealAt("SAT", "2026-09-12", 0, 0));
    ...
    void apply_removesHolidayDepartures_byDefault() {
        // 둘 다 평일(월/화)로 골라서 주말 필터에 안 걸리게 한다.
        List<FlightDealResponse> items =
                List.of(dealAt("HOL", "2026-09-14", 0, 0), dealAt("NORMAL", "2026-09-15", 0, 0));
        when(holidayRepository.findHolidayDatesBetween(LocalDate.of(2026, 9, 14), LocalDate.of(2026, 9, 15)))
                .thenReturn(List.of(LocalDate.of(2026, 9, 14)));
    ...
    void apply_skipsHolidayLookup_whenIncludeHolidayTrue() {
        List<FlightDealResponse> items = List.of(dealAt("A", "2026-09-14", 0, 0));
