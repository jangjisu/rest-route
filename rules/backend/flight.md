...
- `flight_airline.is_low_cost`는 Travelpayouts `/data/airlines.json`의
  `is_lowcost` 필드를 코드 기준으로 그대로 가져온다(별도 수동 조사 불필요).
  이 소스에 없는 코드는 저비용 여부를 알 수 없다는 뜻으로 `false`를 기본값으로
  쓴다.
