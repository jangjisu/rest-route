...
관리자 인증(`ROLE_ADMIN`)이 필요한 API로, `rest_stop`↔주유소 자동 매칭이 놓치거나 잘못 연결한 항목을 점검·수정한다. 사용자에게 보이는 업체·가격 정보는 `rest_oil_price`에서 나오므로 화면의 "주유소" 단위와 `oilId`는 가격 행을 사용한다. 다만 연결과 잠금 상태의 기준은 같은 물리적 주유소(자연키 `standardRestCode`/`serviceAreaCode2`)의 `rest_oil` 행 전체이며, `rest_oil_price.restStopServiceAreaCode`는 이 기준에서 파생되어 가격 동기화 후 다시 채워진다.
...
JSON 바디(`serviceAreaCode`)로 해당 주유소를 지정한 휴게소에 연결한다(기존 연결이 있었다면 교체). 같은 물리적 주유소의 `rest_oil` 행 전체가 연결되고 `adminOverridden=true`로 잠겨 이후 자동 매칭이 값을 바꾸지 않으며, 가격 행의 파생 연결 값도 즉시 갱신한다. 주유소 또는 휴게소가 없으면 `404 Not Found`.

#### DELETE /api/admin/oil-stations/{oilId}/link

연결을 해제한다(`restStopServiceAreaCode`를 `null`로). 같은 물리적 주유소의 `rest_oil` 행 전체를 해제하고 `adminOverridden=true`로 잠가 다음 배치가 다시 자동 연결하지 않으며, 가격 행의 파생 연결 값도 즉시 비운다. 주유소가 없으면 `404 Not Found`.

#### DELETE /api/admin/oil-stations/{oilId}/override

같은 물리적 주유소의 `rest_oil` 행 전체에서 잠금을 해제해 다음 자동 매칭 배치부터 다시 갱신 대상이 되게 한다. 연결 값 자체는 즉시 바뀌지 않고 다음 배치 실행 시 재계산된다. 주유소가 없으면 `404 Not Found`.
