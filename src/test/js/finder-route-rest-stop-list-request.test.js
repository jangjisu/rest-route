...
test('목적지 이름만 있으면 이름만 실어 보낸다(칩 경로)', async () => {
...
test('목적지 좌표가 있으면 좌표와 표시명을 함께 실어 보낸다(검색 후보 선택 경로)', async () => {
    const { calls, fetchImpl } = capturingFetch(200, { code: 'SUCCESS', data: [] });
    const { onState } = collect();
    await createRouteRestStopListRequest({ fetchImpl, onState }).load({
        originLat: 37.0,
...
test('성공 응답이면 loading 후 success와 목록을 낸다', async () => {
    ...
    await createRouteRestStopListRequest({
        ...
        onState
    }).load({ originLat: 37.0, originLng: 127.0, destinationName:'부산역' });
...
test('404 NOT_FOUND는 not-found 상태와 안내 메시지를 낸다', async () => {
    ...
    await createRouteRestStopListRequest({
        ...
        onState
    }).load({ originLat: 37.0, originLng: 127.0, destinationName:'없는곳' });
...
test('뒤이은 요청이 앞선 요청의 응답을 덮어쓰지 않는다', async () => {
    ...
    const firstLoad = request.load({ originLat: 37.0, originLng: 127.0, destinationName:'옛날목적지' });
    await request.load({ originLat: 37.0, originLng: 127.0, destinationName:'새목적지' });
    resolveFirst();
    await firstLoad;
