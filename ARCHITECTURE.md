...
- `rest_stop`↔`rest_oil` 자동 매칭(`route_no = route_code` + 정규화 이름 일치)은 `rest_oil`을 휴게소-주유소 연결과 관리자 잠금 상태의 기준으로 사용한다. 관리자 화면은 사용자에게 보이는 업체·가격 정보가 담긴 휴게소당 0~1건의 `rest_oil_price`를 목록과 검색 단위로 사용하지만, 연결·해제·잠금 해제는 같은 자연키(`standardRestCode`/`serviceAreaCode2`)의 `rest_oil` 행 전체에 기록한다. `rest_oil_price`는 `adminOverridden`을 별도로 소유하지 않고 `rest_oil.standard_rest_code = rest_oil_price.service_area_code2` 관계로 `restStopServiceAreaCode`를 파생한다. `RestOilPriceSyncService`는 전체 페이지 성공 시 가격 행을 교체하고 일부 페이지 성공 시 upsert한 뒤, 저장과 같은 트랜잭션에서 `RestOilPriceServiceAreaCodeBackfiller`를 호출해 기준 연결을 복구한다. 정기·시작 backfill도 같은 컴포넌트를 사용하므로 가격 동기화와 관리자 매핑이 서로 다른 잠금 상태를 만들지 않는다.
